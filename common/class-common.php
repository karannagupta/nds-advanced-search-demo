...
class Common {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'colorbox', plugin_dir_url( __FILE__ ) . 'css/colorbox.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'jquery.colorbox-min', plugin_dir_url( __FILE__ ) . 'js/jquery.colorbox-min.js', array( 'jquery' ), $this->version, true );
		wp_enqueue_script( 'nds_advanced_search', plugin_dir_url( __FILE__ ) . 'js/nds-advanced-search.js', array( 'jquery', 'jquery-ui-autocomplete', 'jquery-ui-datepicker' ), $this->version, true );
	...
	public function shortcode_nds_advanced_search( $atts, $content = null ) {
		...
		/**
		 * Example of the contents of the $_POST[$this->plugin_name] array after the search form is submitted
			Array
			(
				[search_key] => lorem ipsum
				[acf_video_metadata] => Array
					(
						[video_language] => French
						[video_duration] => Short ( less than 15 mins )
						[video_from_date] => January 1, 2017
						[video_to_date] => December 19, 2017
					)
		...
		$user_selected_video_meta = isset( $get_form_input['acf_video_metadata'] ) ? wp_unslash( $get_form_input['acf_video_metadata'] ) : array();
